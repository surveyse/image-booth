...
آدرس: `panel.html` (مثلاً `http://localhost:3000/panel.html`)

- عکس‌های گرفته‌شده از `localStorage` خوانده می‌شوند (بعد از capture در همان مرورگر)
- ورود محلی با ایمیل/رمز در `panel-config.js` (فعلاً `tahmanage@gmail.com`)
- جزئیات هر رکورد: `response-detail.html`

```js
window.IMAGE_PANEL_AUTH = {
  mode: 'local',
  email: 'tahmanage@gmail.com',
  password: '112233'
};
```
...
| `app.js` | دوربین مخفی، capture، آپلود، ذخیرهٔ رکورد برای پنل |
| `config.js` | تنظیمات Cloudinary |
| `panel.html` / `panel.js` | پنل مشاهدهٔ عکس‌ها |
| `panel-config.js` | Panel auth (local email/password) |
| `response-detail.html` | جزئیات یک رکورد |
